          ...
          {slots.map((slot) => (
            <div key={slot.id} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-6 flex flex-col">
              <div className="mb-4 flex-1">
                <h3 className="text-xl font-bold text-gray-900 line-clamp-2">{slot.course_name}</h3>
                <p className="text-sm text-gray-500 font-medium mt-1">{slot.course_code}</p>
              </div>
              ...
              <div className="space-y-2 mb-6">
                <div className="flex items-center text-sm text-gray-700">
                  ...
                  {formatTimeInfo(slot.start_time, slot.end_time)}
                </div>
                <div className="flex items-center text-sm text-gray-700">
                  <svg className="w-4 h-4 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                onClick={() => router.push(`/dashboard/attendance/session?course_id=${slot.course_id}`)}
